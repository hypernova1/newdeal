...
// WebApplicationInitializer 구현체는 웹 애플리케이션이 시작될 때 호출됨
...
public class MyWebApplicationInitializer implements WebApplicationInitializer {
  ...
  public void onStartup(ServletContext servletContext) throws ServletException {
    // 이 메서드가 호출 될 때 할 일
    // 1) 프론트 컨트롤러가 사용할 Spring IoC 컨테이너를 준비
    ...
    // 2) 프론트 컨트롤러를 서블릿 컨테이너에 등록
    // => 프론트 컨트롤러를 생성할 때 각체가 사용할 Spring IoC 컨테이너를 주입
    DispatcherServlet servlet = new DispatcherServlet(iocContainer);
    
    // 서블릿 컨테이너에 위에 만든 프론트 컨트롤러 서블릿 등록
